...
class DialogElectra(ElectraPreTrainedModel):
    ...
    def set_pretrain_electra_weights(self, config):
        self.electra = ElectraModel.from_pretrained(config)
    ...
    def forward(
        ...
        return ((context_word_level, sa_word_level)) + outputs[2:]


class DialogElectraModelingHead(nn.Module):
    def __init__(self, config, num_rnn=1):
        super().__init__()
    ...
    def forward(self, x, sep_pos):
        last_seps = []
        for i in range(sep_pos.size(0)):
            last_sep = 1
            while last_sep < len(sep_pos[i]) and sep_pos[i][last_sep] != 0: 
                last_sep += 1
        ...
        context_word_level, sa_word_level = x

        context_utterance_level = []
        sa_utterance_level = []

        for i in range(context_word_level.size(0)):
...
class ElectraGeneratorPredictions(nn.Module):
    ...
    NOTE: Borrowed from Huggingface Source Code.
    """
...
class DialogElectraMaskedLM(ElectraPreTrainedModel):
    def __init__(self, config, num_decoupling=1):
        super().__init__(config)
        self.config = config
        
        self.dialog_electra = DialogElectra(config, num_decoupling)
    ...
    def set_pretrain_electra_weights(self, config):
        self.electra = self.dialog_electra.set_pretrain_electra_weights(config)

    def get_input_embeddings(self):
        return self.dialog_electra.get_input_embeddings()

    def set_input_embeddings(self, value):
        self.dialog_electra.set_input_embeddings(value)
    ...
    def forward(
        ...
        word_level_features = torch.cat([outputs[0], outputs[1]], dim=-1)
        prediction_scores = self.generator_predictions(word_level_features)
...
class DialogElectraScoring(ElectraPreTrainedModel):
    def __init__(self, config, num_decoupling=1, num_rnn=1):
        super().__init__(config)
        self.config = config
        self.dialog_electra = DialogElectra(config, num_decoupling)
        self.dialog_modeling = DialogElectraModelingHead(config, num_rnn)
        self.cls = nn.Linear(config.hidden_size, 1)

    def set_pretrain_electra_weights(self, config):
        self.dialog_electra.set_pretrain_electra_weights(config)

    def load_pretrain_weights(self, dialog_electra_weights=None, dialog_modeling_weights=None):
        if dialog_electra_weights is None and dialog_modeling_weights is None:
            print("warning: no weights loaded.")
        if dialog_electra_weights:
            self.dialog_electra.load_state_dict(dialog_electra_weights)
        if dialog_modeling_weights:
            self.dialog_modeling.load_state_dict(dialog_modeling_weights)
    ...
    def forward(self,
        ...
        sep_pos = sep_pos.view(-1, sep_pos.size(-1)) if sep_pos is not None else None
        dialog_feats = self.dialog_modeling((outputs[0], outputs[1]), sep_pos)
        logits = self.cls(dialog_feats)
        ...
        if labels is not None:
            ...
            loss = loss_fct(reshaped_logits.squeeze(-1), labels.float())
            outputs = (loss,) + outputs
        return outputs


class DialogElectraNUP(ElectraPreTrainedModel):
    def __init__(self, config, num_decoupling=1, num_rnn=1):
        super().__init__(config)
        self.config = config
        self.dialog_electra = DialogElectra(config, num_decoupling)
    ...
    def load_pretrain_weights(self, dialog_electra_weights=None, dialog_modeling_weights=None):
        ...
        if dialog_electra_weights:
            self.dialog_electra.load_state_dict(dialog_electra_weights)
            print("dialog_electra weights loaded")
        if dialog_modeling_weights:
            self.dialog_modeling.load_state_dict(dialog_modeling_weights)
            print("dialog_modeling weights loaded")
...
class DialogElectraMultipleChoice(ElectraPreTrainedModel):
    def __init__(self, config, num_decoupling=1, num_rnn=1):
        super().__init__(config)
        self.config = config
        self.dialog_electra = DialogElectra(config, num_decoupling)
        ...
        self.cls = nn.Linear(config.hidden_size, 2)
        self.cls_m = nn.Linear(config.hidden_size, 1)
    ...
    def forward(self,
        ...
        dialog_feats = self.dialog_modeling((outputs[0], outputs[1]), sep_pos)
        if num_labels > 2:
            logits = self.cls_m(dialog_feats)
        else:
            logits = self.cls(dialog_feats)
