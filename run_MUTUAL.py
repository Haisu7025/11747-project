...
from modeling import DialogElectraMultipleChoice
...
def convert_examples_to_features_mlm(examples, max_seq_length, max_utterance_num,
    ...
    for (ex_index, example) in enumerate(tqdm(examples, position=0, leave=True)):
        ...
        # for NUP task, text_a is an utterance
...
def main():
    ...
    parser.add_argument("--dialog_electra_weights",
                        default='',
                        type=str)
    parser.add_argument("--dialog_modeling_weights",
                        default='',
                        type=str)
    ...
    parser.add_argument("--cache_data",
                        default='data/mutual_cache_data.pk',
                        type=str,
    ...
    with open("mutual_results.txt", 'a') as f:
        f.write(str(args) + "\n")
    ...
    if os.path.exists(args.cache_data):
        ...
        train_examples = []
        dev_examples = []
        with open("data/mutual_train.json", 'r') as f:
            for line in f.readlines():
                obj = json.loads(line)
                article = obj["article"].replace("m :", "M:").replace("f :", "F:")
                article = re.split(r"(f : |m : |M: |F: )", article)
                article = ["".join(i) for i in zip(article[1::2], article[2::2])]
                truth = str(ord(obj['answers']) - ord('A'))
                options = obj['options']
        ...
        with open("data/mutual_dev.json", 'r') as f:
            for line in f.readlines():
                obj = json.loads(line)
                article = obj["article"].replace("m :", "M:").replace("f :", "F:")
                article = re.split(r"(f : |m : |M: |F: )", article)
    ...
    model = DialogElectraMultipleChoice(config=model_config)
    model.set_pretrain_electra_weights("google/electra-large-discriminator")

    # load pre-trained weights   
    w1, w2 = None, None
    if os.path.exists(args.dialog_electra_weights):
        w1 = torch.load(args.dialog_electra_weights)
    if os.path.exists(args.dialog_modeling_weights):
        w2 = torch.load(args.dialog_modeling_weights)
    model.load_pretrain_weights(w1, w2)
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        with tqdm(train_dataloader, desc="Training", position=0, leave=True) as t:
            for step, batch in enumerate(t):
                ...
                t.set_postfix({"avg loss": tr_loss / (step + 1), "loss": closs})
                t.update()
        ...
        with tqdm(eval_dataloader, desc="Validation", position=0, leave=True) as t:
            ...
            with open("mutual_results.txt", 'a') as f:
                f.write(str(epoch) + "\n")
                f.write(str(result))
        model.save_pretrained(f"output/electra_mutual_{epoch}")
